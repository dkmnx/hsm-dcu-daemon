 ...
 *
 *    Description:
 *      This file implements "commissioner" command in wfanctl.
 *
 *      The wfanctl "commissioner" command is implemented in `tool-cmd-commr.c`. This file contains the old
	...
	while (1) {
		...
		case 's':
			...
			if (gInterfaceName[0] == 0) {
				fprintf(stderr,
						"%s: error: No WPAN interface set (use the `cd` command, or the `-I` argument for `wfanctl`).\n",
						argv[0]);
