...
#include "wfanctl-cmds.h"
#include "version.h"
#include "string-utils.h"
#include "wfanctl-utils.h"
...
char* wfanctl_generator(const char* text, int state)
{
	static int idx, len;
	const char *name;
...
static char** wfanctl_completion(const char *text, int start, int end)
{
	if (start != 0)
		return NULL;

	return rl_completion_matches(text, wfanctl_generator);
}
#endif // HAVE_LIBREADLINE
...
initialize_readline()
{
	...
	rl_attempted_completion_function = wfanctl_completion;
...
wpan_dbus_version_check(DBusConnection* connection)
{
	...
	if (gDebugMode >= 1) {
		fprintf(stderr, "DEBUG: Version check, wfanctl=%d, wpantund=%d\n", WPAN_TUNNEL_DBUS_VERSION, version);
	}

	if(version != WPAN_TUNNEL_DBUS_VERSION) {
		fprintf(stderr, "error: `wpantund` version (%d) doesn't match `wfanctl` version (%d).\n", version, WPAN_TUNNEL_DBUS_VERSION);
		goto bail;
	}
...
int main(int argc, char * argv[])
{
	...
	while (1) {
		...
		if ((optind < argc) && (find_cmd(argv[optind]) != NULL)) {
			// This is where the wfanctl command starts; skip
	...
	if (gRet != 0) {
		fprintf(stderr,
		        "%s: error: `wpantund` is either not running, locked up, or incompatible with this version of `wfanctl`.\n",
		        argv[0]
		        );
	...
	if (istty) {
		...
		setenv("WPANCTL_HISTORY_FILE", tilde_expand("~/.wfanctl_history"), 0);
