...
When developing, it can be necessary to restart the server frequently while making changes. Since this app manages dcud, it can cause dcud errors if left in the default mode. To fix this, add the `-d` or `--dev-mode` option. This prevents the app from managing dcud, so you will need to run dcud separately in another terminal (e.g. `sudo dcud -s /dev/ttyACM0`).

Another option provided for this server is to use a different serial port from the default /dev/ttyACM0. To do this, add the `-s` or `--serial-port` option followed by the serial port (e.g. `/dev/ttyS0`). Other options are specified in `<base dcud directory>/ti-wisun-webapp/server/src/AppConstants.js`.
...
Logs generated by **dcud** and the webserver are dumped in the `/tmp/ti-wisun-webapp/logs` directory.
