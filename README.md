...
Refer to Install.md for compiling and installing `wfantund` and `dcuctl` 
in Ubuntu Linux OS.
...
### Configuring wfantund using dcuctl (Alternate) ###

To use `dcuctl`, first start wfantund:
...
When wfantund is already running, you can also check the daemon version in dcuctl using the `status` command.
...
### Using wfantund and dcuctl with Wi-SUN router node ###

While the Linux webserver can only be used with Wi-SUN border routers, wfantund and dcuctl can also be used
...
The steps to using wfantund and dcuctl for router nodes are the same. The only difference is, once the interface
...
`wfantund` includes a command line utility called `dcuctl`.
