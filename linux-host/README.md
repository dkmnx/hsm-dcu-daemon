Linux Host Overview
-------------------------------
The Linux Host is a port of the upper layers of the Wi-SUN MAC and the Application layer for the Border Router and Router Node. It is designed to connect to a TI device running the RCP LMAC Firmware. To support this new functionality, dcud has also been updated to connect to a TCP socket instead of only a serial port. The Linux Host application will listen on port 4902 for incoming connections which dcud can use to communicate with the application. 
...
3. Start up the containers with `docker compose up -d`. This will start the `linux_host` container and the `dcud` container.
...
1. Open a shell into the `dcud` container with `docker compose exec -it dcud bash`.
2. Build and install `dcud` by running `./bootstrap.sh && ./configure --sysconfdir=/etc && make install`. Next, make sure that dbus is started by running `service dbus start`. Once these steps are done, there's no need to rebuild it again or restart dbus unless you restart the container.
3. If running border router, start dcud by running `dcud -o Config:NCP:SocketPath tcp:linux_host:4902 -o IPv6:WfantundGlobalAddress 2020:ABCD::/64`. This will create the wfan0 interface, assign it address 2020:ABCD::, and create a TCP connection to the `linux_host` container. If running router node, start dcud by running `dcud -o Config:NCP:SocketPath tcp:linux_host:4903`, similarly creating the wfan0 interface and TCP linux_host container connection.
4. Start another shell session with `docker compose exec -it dcud bash` and start dcuctl by running `dcuctl`. From here you can interact with the interface as normal; start the stack by running `set interface:up true` in dcuctl.
5. When using external servers, start a shell session for each of them with `docker compose exec -it dcud bash`. To start FreeRADIUS in the foreground, run `freeradius -X`. To start dnsmasq in the foreground, run `dnsmasq -d -i wfan0 --dhcp-range 2020:abcd::1,2020:abcd::ffff,64,336h`.
...
This issue is usually caused by dcud hanging on to the TCP port after the Linux Host goes down and is brought back up. If this has happened, be sure to stop both the Linux Host and the dcud processes, give it a minute or two for the kernel to clean everything up, and start again. If you need to bring the Linux Host down, stop the dcud process first and then stop the Linux Host to prevent this issue.

PySpinel Notes
--------------------

PySpinel can also be used to configure and start the Linux Host application. This can be done by invoking PySpinel with `python3 spinel-cli.py -s <NCP Socket Number>`. Out of the box, this would look like `python3 spinel-cli.py -s 4902` to connect to the Linux Host Border Router. Note that right now PySpinel will only connect to a port open on `localhost`, so PySpinel must be run either directly in the linux_host or dcud container, or connected to the network in its own container.

Untested Features
--------------------

The webserver has not been tested with the Linux Host, but should work the same as long as it also runs in the dcud container. 
